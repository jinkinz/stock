...
def _clamp_to_budget(quantity: float, price: float, max_trade_value: float,
                     spendable: float) -> tuple[float, str]:
    ...
    cash_cap = spendable * MAX_CASH_FRACTION_PER_TRADE
    if quantity * price > cash_cap:
        quantity = cash_cap / price
        note = f"capped at {MAX_CASH_FRACTION_PER_TRADE:.0%} of available cash"
...
def size_position(price: float, atr: float, equity: float, spendable: float,
                  ...
                  atr_stop_multiple: float, use_atr_sizing: bool = True) -> SizingResult:
    ...
    quantity, cap_note = _clamp_to_budget(quantity, price, max_trade_value, spendable)
